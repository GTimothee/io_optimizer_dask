...
def time_to_read_and_write_buffer():
    """
    from hdf5 file as input
    to npy files as output or to hdf5 as output
    """

    def load_data_in_memory(arr):
        with Profiler() as prof, ResourceProfiler() as rprof, CacheProfiler(metric=nbytes) as cprof:    
            t = time.time()
            raw_arr = arr.compute()
            print(f'time to load the array in memory from hdf5: {time.time() - t}')
            visualize([prof, rprof, cprof], out_file_path)
        return raw_arr

    # Creating dask array object from hdf5
    filepath = "/run/media/user/HDD 1TB/data/1.hdf5"
    chunks_shape = (175, 1400, 1400)
    
    # FIRST EXP
    print(f'1) First experiment saving directly the data after having loaded it in memory.')

    # npy using numpy save function
    flush_cache()
    arr = get_dask_array_from_hdf5(file_path, logic_chunks_shape=chunks_shape)
    raw_arr = load_data_in_memory(arr)
    with Profiler() as prof, ResourceProfiler() as rprof, CacheProfiler(metric=nbytes) as cprof:    
        t = time.time()
        np.save('data/out_1.npy', raw_arr)
        print(f'time to save array in numpy stack: {time.time() - t}')
        visualize([prof, rprof, cprof], 'outputs/1.html')
    del arr

    # hdf5 using dask function
    flush_cache()
    arr = get_dask_array_from_hdf5(file_path, logic_chunks_shape=chunks_shape)
    raw_arr = load_data_in_memory(arr)
    with Profiler() as prof, ResourceProfiler() as rprof, CacheProfiler(metric=nbytes) as cprof:    
        t = time.time()
        da.to_hdf5('data/out_2.hdf5', 'data', raw_arr, chunks=None)
        print(f'time to save the array to hdf5 file (1 dataset): {time.time() - t}')
        visualize([prof, rprof, cprof], 'outputs/2.html')
    del arr

    # SECOND EXP
    print(f'2) Same experiment saving directly the data without intermediary data loading')

    # npy using dask's numpy stack function
    flush_cache()
    arr = get_dask_array_from_hdf5(file_path, logic_chunks_shape=chunks_shape)
    with Profiler() as prof, ResourceProfiler() as rprof, CacheProfiler(metric=nbytes) as cprof:    
        t = time.time()
        da.to_npy_stack('data/out_3_numpy', arr, axis=0)
        print(f'time to save array in numpy stack: {time.time() - t}')
        visualize([prof, rprof, cprof], 'outputs/3.html')
    del arr
    ...
    with Profiler() as prof, ResourceProfiler() as rprof, CacheProfiler(metric=nbytes) as cprof:    
        t = time.time()
        da.to_hdf5('data/out_4.hdf5', 'data', arr, chunks=None)
        print(f'time to save the array to hdf5 file (1 dataset): {time.time() - t}')
        visualize([prof, rprof, cprof], 'outputs/4.html')
    del arr
