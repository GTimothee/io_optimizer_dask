...
def test_sum():
    ...
    nb_arr_to_sum = 2
    for chunk_shape in test_utils.chunk_shapes:
        # non opti
...
def test_adding_chunks(computation=True, visuals=True, run_non_optimized=True, run_optimized=True):
    ...
    def do_a_run(message, number_of_arrays, viz, prefix=None, suffix=None):
        results = list()
        for chunk_shape in test_utils.chunk_shapes:
    ...
    if run_non_optimize:  # if both have been ran, we can compare their results
